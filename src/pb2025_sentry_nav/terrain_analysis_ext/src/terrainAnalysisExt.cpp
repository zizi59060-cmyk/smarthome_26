...
void downsampleCloud(
  ...
  if (!cuda_pointcloud_preprocessor::cudaAvailable()) {
    if (!loggedCpuFallback) {
      RCLCPP_INFO(logger, "terrain_analysis_ext voxel downsample using PCL CPU fallback");
      loggedCpuFallback = true;
    }
    downSizeFilter.setInputCloud(input);
    downSizeFilter.filter(*output);
    return;
  }

  cudaInput.clear();
  cudaInput.reserve(input->points.size());
  for (const auto &point : input->points) {
    cudaInput.push_back(cuda_pointcloud_preprocessor::PointXYZI{
        point.x, point.y, point.z, point.intensity, 0.0f});
  }

  bool usedCuda = false;
  cuda_pointcloud_preprocessor::voxelDownsample(
      cudaInput, cudaOutput, static_cast<float>(scanVoxelSize), true, &usedCuda);
  ...
  if (!loggedCuda) {
    RCLCPP_INFO(logger, "terrain_analysis_ext voxel downsample using CUDA");
    loggedCuda = true;
  }

  output->clear();
  output->reserve(cudaOutput.size());
    ...
    if (newlaserCloud) {
      ...
      for (int ind = 0; ind < kTerrainVoxelNum; ind++) {
        if (terrainVoxelUpdateNum[ind] >= voxelPointUpdateThre ||
          ...
          laserCloudDwz->clear();
          downsampleCloud(terrainVoxelCloudPtr, laserCloudDwz, nh->get_logger());
