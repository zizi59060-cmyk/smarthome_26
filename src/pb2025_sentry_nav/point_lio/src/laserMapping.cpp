...
bool downsampleSurfCloudWithCuda()
{
  static std::vector<cuda_pointcloud_preprocessor::PointXYZI> cudaInput;
  static std::vector<cuda_pointcloud_preprocessor::PointXYZI> cudaOutput;
  static bool loggedCuda = false;
  static bool loggedCpuFallback = false;

  if (!cuda_pointcloud_preprocessor::cudaAvailable()) {
    if (!loggedCpuFallback) {
      RCLCPP_INFO(LOGGER, "point_lio scan voxel downsample using PCL CPU fallback");
  ...
  cudaInput.clear();
  cudaInput.reserve(feats_undistort->points.size());
  for (const auto & point : feats_undistort->points) {
    cudaInput.push_back(cuda_pointcloud_preprocessor::PointXYZI{
      point.x, point.y, point.z, point.intensity, point.curvature});
  }

  bool usedCuda = false;
  cuda_pointcloud_preprocessor::voxelDownsample(
    cudaInput, cudaOutput, static_cast<float>(filter_size_surf_min), true, &usedCuda);
  if (!usedCuda) {
    if (!loggedCpuFallback) {
      RCLCPP_INFO(LOGGER, "point_lio scan voxel downsample using PCL CPU fallback");
      loggedCpuFallback = true;
  ...
  if (!loggedCuda) {
    RCLCPP_INFO(LOGGER, "point_lio scan voxel downsample using CUDA");
    loggedCuda = true;
  }

  feats_down_body->clear();
  feats_down_body->reserve(cudaOutput.size());
  for (const auto & point : cudaOutput) {
    PointType out;
    out.x = point.x;
    out.y = point.y;
    out.z = point.z;
    out.intensity = point.intensity;
    out.curvature = point.time;
    out.normal_x = 0.0f;
    out.normal_y = 0.0f;
    out.normal_z = 0.0f;
    feats_down_body->push_back(out);
  }
  ...
  while (rclcpp::ok()) {
    ...
    if (sync_packages(Measures)) {
      ...
      if (space_down_sample) {
        if (!downsampleSurfCloudWithCuda()) {
