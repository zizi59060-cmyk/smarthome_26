...
std::string resolvePackagePath(const std::string & path)
{
  ...
  const auto package_share = ament_index_cpp::get_package_share_directory("smarthome_vision");
  return (std::filesystem::path(package_share) / model_path).string();
}
...
public:
  VisionNode() : Node("smarthome_vision_node")
  {
    ...
    object_detector_ = std::make_unique<Detector>(
      resolvePackagePath(get_parameter("object_model_path").as_string()),
    ...
    if (qr_recognition_method_ != 1) {
      qr_detector_ = std::make_unique<Detector>(
        resolvePackagePath(get_parameter("qr_model_path").as_string()),
