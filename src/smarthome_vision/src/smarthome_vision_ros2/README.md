...
| --- | --- | --- |
| `0` | `IDLE` | 空闲 |
| `1` | `DETECT_OBJECT` | 使用 OpenVINO 模型识别物体 |
| `2` | `DETECT_QR` | 默认使用传统视觉识别二维码 |
...
| `openvino_device` | OpenVINO 推理设备，默认 `CPU` |
| `object_model_path` | 物体识别 OpenVINO 模型路径，可用 `.onnx` 或 IR `.xml` |
| `object_model_class_ids` | 物体模型输出类别到工程类别的映射 |
| `qr_model_path` | 二维码 OpenVINO 模型路径；`qr_recognition_method=1` 时不使用 |
| `qr_model_class_ids` | 二维码模型输出类别到工程类别的映射 |
| `qr_size` | 二维码物理尺寸 |

实车换机器后最常改的是 `object_model_path`、`qr_recognition_method`、`openvino_device`、相机内参和摄像头编号。模型路径可以写绝对路径，也可以写相对 package share 的路径，例如 `assets/best-dynamic.xml`。OpenVINO 可直接读取 `.onnx`；如果使用 IR 模型，需要把匹配的 `.xml` 和 `.bin` 一起放到 `assets/` 或配置指向的位置。
...
| 改 OpenVINO 2024.3 推理 | `src/openvino_detector.cpp` |
